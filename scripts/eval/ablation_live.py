...
# The repaired audit widened the STATIC baseline's batch axis after finding that
# the historical {4, 8, 16} search could under-select Static-Best.
SEARCH_SPACE_WIDE = dict(
    SEARCH_SPACE,
    max_batch_size=[2, 4, 8, 12, 16, 24, 32],
)
SEARCH_SPACES = {
    "restricted": SEARCH_SPACE,
    "wide": SEARCH_SPACE_WIDE,
}
N_LHS_CANDIDATES = 16

# CARL-Expanded changes only action-space reach. It keeps the other knobs at the
# regime default for every newly introduced batch-size arm.
ARM_SET_BATCH_AXIS = list(SEARCH_SPACE_WIDE["max_batch_size"])


def expanded_arm_sets() -> dict:
    """Return shipped arms plus one arm for each unreachable batch size.

    ``all_arm_sets()`` returns fresh configs, so this helper never mutates the
    shipped action space.
    """
    out = {}
    for regime, arms in all_arm_sets().items():
        arms = list(arms)
        reachable = {a.max_batch_size for a in arms}
        base = DEFAULT_CONFIGS[regime]
        for mb in ARM_SET_BATCH_AXIS:
            if mb not in reachable:
                arms.append(replace(base, max_batch_size=mb).clamp())
        out[regime] = arms
    return out


ARM_SETS = {
    "restricted": all_arm_sets,
    "expanded": expanded_arm_sets,
}


def arm_set_summary(arms_by_regime: dict) -> dict:
    """Per-regime action-space reach and arm count for result provenance."""
    return {
        regime.value: {
            "n_arms": len(arms),
            "max_batch_sizes": sorted({a.max_batch_size for a in arms}),
        }
...
# Repair treatments are opt-in so the historical ablation remains reproducible.
_ARM_SET_FOR = {
    "CARL-Expanded": "expanded",
}
_BANDIT_CLS_FOR = {
    "CARL-Repaired": RepairedLinUCBBandit,
    "CARL-Expanded": RepairedLinUCBBandit,
}
_TREATMENTS = ("CARL-Repaired", "CARL-Expanded")


def _bandit_cls_name(name: str) -> str:
    return _BANDIT_CLS_FOR.get(name, LinUCBBandit).__name__


# Configs whose frozen knob actually changes live inference in this harness.
_LIVE_EFFECTIVE = {
    "CARL-Full", "CARL-NoSched", "CARL-NoChunk",
...
def scenario_description(n: int) -> str:
    """Generate the NON-STATIONARY label from the actual request count.

    ``_build_workload`` splits the run at ``n // 2``. The old literal described
    only the n=50 default and therefore mislabeled larger hardening runs.
    """
    half = n // 2
    return (
        f"NON-STATIONARY (1-{half} INTERACTIVE prompt16-64/max32, "
        f"{half + 1}-{n} BATCH prompt128-256/max64)"
    )
...
def _frozen_arms(freeze: dict | None, arm_set: str = "restricted") -> dict:
    """Return the selected action space, optionally with one knob frozen."""
    base = ARM_SETS[arm_set]()
    if not freeze:
        return base
    return {
        r: [replace(a, **freeze).clamp() for a in arms]
        for r, arms in base.items()
    }
...
def run_config(name, model, tokenizer, n, seed, *, static_cfg=None,
    ...
    if name == "Static-Best":
        ...
        if name == "CARL-Thompson":
            ...
            arm_set = _ARM_SET_FOR.get(name, "restricted")
            bandit_cls = _BANDIT_CLS_FOR.get(name, LinUCBBandit)
            bandit = PerRegimeBandit(
                _frozen_arms(_FREEZE.get(name), arm_set),
                d=FEATURE_DIM,
    ...
    if controller is not None:
        ...
        for e in controller.controller_log:
            ...
            decisions.append({
                "regime": e.regime,
                "arm": _arm_index(arms, e.config),
                "reward": e.reward,
                "config": e.config,
                "t_monotonic_s": getattr(e, "t_monotonic_s", 0.0),
            })
        out["decisions"] = decisions
        out["arm_set"] = _ARM_SET_FOR.get(name, "restricted")
        out["bandit_cls"] = type(
            controller.bandit.bandits[next(iter(controller.bandit.bandits))]
        ).__name__
    return out
...
def select_static_best(
    ...
    space_name: str = "restricted",
) -> tuple:
    """Pick Static-Best on held-out validation without changing old callers."""
    space = SEARCH_SPACE if space is None else space
    print(
        f"\n[validation] LHS {N_LHS_CANDIDATES} candidates x {val_n} requests "
        f"(seed {VALIDATION_SEED}, space={space_name}, "
        f"max_batch_size={space['max_batch_size']})",
        flush=True,
    )
    candidates = latin_hypercube(N_LHS_CANDIDATES, space, VALIDATION_SEED)
    throughputs = []
    ...
    selection = {
        ...
        "search_space": space,
        "search_space_is_wide": space is SEARCH_SPACE_WIDE,
...
def run_all(
    ...
    static_cfg, selection = select_static_best(
        model,
        tokenizer,
        max(10, n // 2),
    ...
    results: dict = {
        "environment": env,
        "scenario": scenario_description(n),
        ...
        "arm_sets": {
            "restricted": arm_set_summary(all_arm_sets()),
            "expanded": arm_set_summary(expanded_arm_sets()),
            "expanded_batch_axis": ARM_SET_BATCH_AXIS,
            "note": (
                "restricted = src.carl.config.all_arm_sets() as shipped; "
                "expanded = restricted plus one arm per unreachable "
                "max_batch_size, each being that regime's DEFAULT_CONFIG with "
                "only max_batch_size replaced. Restricted is a strict subset "
                "of expanded; no other knob gains a new value."
            ),
        },
        "config_arm_sets": {
            name: _ARM_SET_FOR.get(name, "restricted")
            for name in list(CONFIGS) + list(_TREATMENTS)
        },
        "config_bandit_cls": {
            name: _bandit_cls_name(name)
            for name in list(CONFIGS) + list(_TREATMENTS)
        },
        "treatments": {
            "CARL-Full": {
                "learner": "LinUCBBandit",
                "arm_set": "restricted",
                "note": "the existing ablation config, unchanged",
            },
            "CARL-Repaired": {
                "learner": "RepairedLinUCBBandit",
                "arm_set": "restricted",
                "note": "differs from CARL-Full in the LEARNER only",
            },
            "CARL-Expanded": {
                "learner": "RepairedLinUCBBandit",
                "arm_set": "expanded",
                "note": "differs from CARL-Repaired in the ARM SET only",
            },
        },
    ...
    at = run_configs.index("CARL-Full") + 1
    for extra, wanted in (
        ("CARL-Repaired", include_repaired),
        ("CARL-Expanded", include_expanded),
    ):
        if wanted and extra not in run_configs:
            run_configs.insert(at, extra)
            at += 1
    results["configs_run"] = run_configs
    ...
    for name in run_configs:
        ...
        agg = {
            "live_effective": name in _LIVE_EFFECTIVE,
            "arm_set": _ARM_SET_FOR.get(name, "restricted"),
            "bandit_cls": _bandit_cls_name(name),
        }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "also run CARL-Repaired, which differs from CARL-Full ONLY in the "
            "learner (RepairedLinUCBBandit: intercept feature + reward centring). "
            "Off by default so the existing ablation is unchanged."
        ),
    )
    parser.add_argument(
        "--include-expanded",
        action="store_true",
        help=(
            "also run CARL-Expanded, which differs from CARL-Full ONLY in the "
            "bandit's arm set (max_batch_size in {2,4,8,12,16,24,32}) and uses "
            "the repaired learner. Off by default so the existing ablation is unchanged."
        ),
    )
    parser.add_argument(
        "--search-space",
        ...
        default="wide",
        help=(
            "Static-Best validation search space. 'wide' uses the validated batch "
            "axis {2,4,8,12,16,24,32}; 'restricted' reproduces historical "
            "{4,8,16}. This is the BASELINE's space, not CARL's arm set."
        ),
    )
