...
def test_controller_log_records_each_decision():
    ...
    # Each entry carries a JSON-friendly view plus reward/provenance metadata.
    d = c.controller_log[0].as_dict()
    assert set(d) >= {
        "step", "regime", "config", "reward", "t_monotonic_s",
        "state_features",
    }
    assert set(d) >= {"arm", "rewarded_arm", "reward_terms", "observed"}
    assert d["reward_terms"] == {}
    assert len(d["state_features"]) == FEATURE_DIM

    # Runtime provenance timestamps are monotonic across the decision log.
    stamps = [e.t_monotonic_s for e in c.controller_log]
...
def test_stats_regime_distribution():
    ...
    assert set(stats) == {
        ...
        "total_adaptations", "best_config_per_regime", "reward_validity",
    }
