...
class ControllerLogEntry:
    ...
    reward_validity: dict = field(default_factory=dict)
    ...
    def as_dict(self) -> dict:
        ...
        return {
            ...
            "t_monotonic_s": self.t_monotonic_s,
            "arm": self.arm,
            "rewarded_arm": self.rewarded_arm,
...
class CARLController:
    def __init__(
        ...
        Args:
            ...
            metrics: the MetricsTracker holding rolling latency/throughput
                ...
                feeds it samples.
            reward_fn: optional reward strategy called as
                ``reward_fn(state, metrics) -> (reward, terms_dict)``.
                When None, the historical reward-v1 utility path is retained.
                An injected strategy (for example LiveRewardV2) may return
                ``None`` to withhold a control-cycle reward until its required
                live metrics have been observed.
        """
        ...
        self._valid_reward_updates = 0
        self._invalid_reward_cycles = 0
        self._first_valid_reward: tuple[int, int, float] | None = None
    ...
    def _reward_for_state(self, state: RuntimeState) -> tuple[float | None, dict]:
        """Map an observed state onto ``(reward, per_term_breakdown)``.

        With no injected reward strategy this preserves the historical reward-v1
        utility path. An injected strategy may return ``None`` when the live
        measurements required to score the cycle do not exist yet. That value is
        deliberately propagated so ``step`` cannot feed a fabricated fill value
        to the learner.
        """
        if self.reward_fn is not None:
            reward, terms = self.reward_fn(state, self.metrics)
            return (None if reward is None else float(reward), dict(terms))

        throughput_norm = (
            min(1.0, state.throughput_tps / self.slo.throughput_ref)
            if self.slo.throughput_ref > 0
            else 0.0
        )
        metrics = {
    ...
    def _observed_metrics(self, state: RuntimeState) -> dict:
        """Raw serving metrics behind one control cycle.

        ``realised_batch_mean`` records what the engine actually ran rather than
        the configured maximum batch size, which is the quantity needed for
        cap-binding analysis.
        """
        return {
            "throughput_tps": state.throughput_tps,
            "ttft_p50_ms": state.p50_ttft_ms,
            "ttft_p99_ms": (
                self.metrics.p99_ttft_ms()
                if hasattr(self.metrics, "p99_ttft_ms")
                else None
            ),
            "tpot_p99_ms": state.p99_tpot_ms,
            "realised_batch_mean": state.batch_size_mean,
    ...
    def step(self, step_idx: int | None = None, state: RuntimeState | None = None):
        ...
        """
        with self._lock:
            t_monotonic_s = time.perf_counter()
            ...
            reward, reward_terms = self._reward_for_state(state)
            reward_valid = reward is not None
            reward_validity = dict(reward_terms.get("reward_validity") or {})
            rewarded_arm = -1

            # Invalid/warm-up cycles are logged but never update LinUCB.
            if reward_valid and self._prev is not None and self.bandit is not None:
            ...
            entry = ControllerLogEntry(
                ...
                observed=self._observed_metrics(state),
            ...
            if reward_valid:
                ...
                if self._first_valid_reward is None:
                    self._first_valid_reward = (
                        len(self.controller_log) - 1,
                        entry.step,
                        t_monotonic_s,
                    )
                if rewarded_arm >= 0:
                    self._valid_reward_updates += 1
            else:
                self._invalid_reward_cycles += 1

            # 5. Remember this cycle for delayed reward on the next cycle.
    ...
    def stats(self) -> dict:
        ...
        ``mean_reward_per_regime`` averages valid/scored cycles only. The
        ``reward_validity`` record carries the corresponding denominators and
        warm-up information so run-level summaries cannot silently count
        withheld cycles as scored observations.
        """
        regime_distribution = {r.value: n for r, n in self._regime_counts.items()}
        mean_reward = {
            r.value: (self._reward_sum[r] / self._reward_n[r])
            for r in self._reward_sum
            if self._reward_n.get(r)
        }
        return {
            ...
            "reward_validity": self.reward_validity_report(),
        }

    def reward_validity_report(self) -> dict:
        """Return the run's reward warm-up/validity accounting.

        Warm-up is the leading prefix of cycles whose reward was withheld because
        required live measurements were not yet available. Once the first valid
        reward appears, validity should remain monotone for the normal growing
        metric windows; ``withheld_after_first_valid`` makes violations visible.
        """
        cycles = self.controller_log
        n = len(cycles)
        n_valid = sum(1 for entry in cycles if entry.reward_valid)
        first = self._first_valid_reward

        warmup = 0
        for entry in cycles:
            if entry.reward_valid:
                break
            warmup += 1

        withheld_after_first_valid = sum(
            1 for entry in cycles[warmup:] if not entry.reward_valid
        )
        first_t = first[2] if first else None
        t0 = cycles[0].t_monotonic_s if cycles else None

        return {
            "cycles": n,
            "valid_reward_cycles": n_valid,
            "withheld_reward_cycles": self._invalid_reward_cycles,
            "warmup_cycles": warmup,
            "withheld_after_first_valid": withheld_after_first_valid,
            "valid_reward_updates": self._valid_reward_updates,
            "first_valid_reward_cycle_index": first[0] if first else None,
            "first_valid_reward_scheduler_step": first[1] if first else None,
            "first_valid_reward_t_monotonic_s": first_t,
            "warmup_duration_s": (
                first_t - t0
                if first_t is not None and t0 is not None
                else None
            ),
            "policy": (
                "a control cycle whose reward metrics were not yet measured is "
                "logged with reward=None and reward_valid=False; the learner "
                "receives no update and no fill value is substituted"
            ),
        }
