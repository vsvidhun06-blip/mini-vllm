...
class MetricsTracker:
    ...
    def p99_ttft_ms(self) -> float:
        """Tail TTFT over the rolling window.

        Additive and deliberately not a context feature. Reward v2 scores
        ``ttft_p99_ms``, while ``RuntimeState`` retains TTFT p50 as the context
        feature. Keeping p99 out of ``_FEATURE_SCALES`` preserves FEATURE_DIM and
        keeps reward-v1 and reward-v2 runs comparable.
        """
        return _percentile(list(self._ttft), 99)
    ...
    # Reward-v2 validity gate. Missing latency samples must not look like
    # perfect latency to the learner.
    MIN_LATENCY_SAMPLES = 1

    def n_ttft_samples(self) -> int:
        """Strictly-positive TTFT observations currently in the window."""
        return sum(1 for v in self._ttft if v > 0)

    def n_tpot_samples(self) -> int:
        """Strictly-positive TPOT observations currently in the window.

        Excludes the ``tpot_ms = 0.0`` sentinel used by single-token requests.
        """
        return sum(1 for v in self._tpot if v > 0)

    def n_throughput_samples(self) -> int:
        return len(self._tps)

    def metric_validity(self, min_samples: int | None = None) -> dict:
        """Report whether the latency metrics consumed by reward v2 are measured.

        Missing latency is dangerous because a zero latency can be interpreted as
        perfect latency. Throughput is reported for provenance but does not gate:
        an unmeasured throughput contributes zero rather than an artificially good
        score, and live reward v2 reads throughput from RuntimeState.
        """
        k = self.MIN_LATENCY_SAMPLES if min_samples is None else int(min_samples)
        n_tpot = self.n_tpot_samples()
        n_ttft = self.n_ttft_samples()
        valid = {
            "ttft_p99_ms": n_ttft >= k,
            "tpot_p99_ms": n_tpot >= k,
        }
        return {
            "min_samples_required": k,
            "gated_metrics": sorted(valid),
            "gate_rationale": (
                "a missing latency percentile is scored as PERFECT latency and "
                "inflates the credited arm; missing throughput is reported but "
                "does not gate because it cannot manufacture a favorable score"
            ),
            "n_ttft_samples": n_ttft,
            "n_tpot_samples": n_tpot,
            "n_throughput_samples": self.n_throughput_samples(),
            "n_ttft_observations_raw": len(self._ttft),
            "n_tpot_observations_raw": len(self._tpot),
            "valid": valid,
            "missing": sorted(m for m, ok in valid.items() if not ok),
